  ---
title: "Heatmap of Disease Association Scores for estrogen DEGs"
...
```{r}
degs <- read.csv("../result/pbmc/estrogen_significant_DEGs.csv", sep = ",", header = T)
gene_list <- degs %>%
...
write.csv(gene_list, "../result/association/estrogen_DEG_asso_score.csv", row.names = TRUE)
```
...
pheatmap(gene_list_filtered[, -1], # Exclude the first column (SYMBOL) from the heatmap data
         main = "Estrogen Gene Association Scores Across Diseases", # Title of the heatmap,
         ...
         cellwidth = 40, # Adjust cell width
         cellheight = 10, # Adjust cell height,
         filename = "../result/association/heatmap_estrogen_DEG_asso_score.png" # Save heatmap to file
)
