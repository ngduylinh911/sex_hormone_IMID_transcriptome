---
title: "DEGs analysis based on testosterone group in PBMC dataset"
...
We will divide sample into 2 groups: high and low testosterone. Using the first quantile and third quantile to group the samples
...
```{r}
metadata.subset <- metadata.subset[!is.na(metadata.subset$testosterone),]
first_quantile <- quantile(metadata.subset$testosterone, probs = 0.25)
third_quantile <- quantile(metadata.subset$testosterone, probs = 0.75)
metadata.subset$group[metadata.subset$testosterone <= first_quantile] = "low"
metadata.subset$group[metadata.subset$testosterone >= third_quantile] = "high"
head(metadata.subset)
```
...
```{r}
keep <- rowSums(counts(dds) >= 10) >= 5 # keep rows with at least 10 counts in at least 5 samples (50% of cohort)
dds <- dds[keep, ]
dds
```
## Annotation
...
write.csv(normalized_counts, file = "../result/pbmc/testosterone_normalized_counts.csv",)
...
So, with an adjusted p-value of 0.05, we identify 25 downregulated genes and 13 upregulated genes in the high testosterone group compared to the low testosterone group.
...
res.mod$delabel <- NA
res.mod$delabel[1:20] <- res.mod$SYMBOL[1:20] # label the top 20 genes
...
volcano_plot <- ggplot(data = res.mod, aes(x=log2FoldChange, y=-log10(padj),color = diffexpres.modsed, label = delabel))+
  ...
  ggtitle("Volcano plot of DEGs between High and Low testosterone level")+
  geom_text_repel(max.overlaps = Inf)
# Save the plot to a file
ggsave("../result/pbmc/img/testosterone_volcano_plot.png", plot = volcano_plot, width = 6, height = 8, dpi = 300)
...
```
So, with an adjusted p-value of 0.05, we identify 307 downregulated genes and 12 upregulated genes in the high testosterone group compared to the low testosterone group.

```{r}
write.csv(res.mod, file = "../result/pbmc/testosterone_DEGs_result.csv", row.names = TRUE) # save the results of all genes to a csv file
```

```{r}
significant_genes <- res.mod[!is.na(res.mod$padj) & res.mod$padj < 0.05 & (res.mod$log2FoldChange >= 1 | res.mod$log2FoldChange <= -1), ]
write.csv(significant_genes, file = "../result/pbmc/testosterone_significant_DEGs.csv", row.names = FALSE) # save the results of only significant genes to a csv file
```
...
### Custom plot counts of the top genes (which label of testosterone level)
...
combined_data <- lapply(top5_genes, function(gene_id) {
  ...
  d$testosterone <- metadata.subset$testosterone[match(rownames(d), rownames(metadata.subset))]
...
pairsplot(p, 
          ...
          colby = 'group',
          lab = round(p$metadata$testosterone),
          labSize = 5,)
```
...
# write.csv(vst_degs_by_pvalue, file = "../result/pbmc/testosterone_vst_DEGs_expression_by_pvalue.csv", row.names = TRUE)
write.csv(metadata.subset, file = "../result/pbmc/testosterone_clinical_data.csv")
```
...
write.csv(all_rld_matrix, file = "../result/pbmc/testosterone_vst_all_genes.csv",row.names = TRUE)
```
