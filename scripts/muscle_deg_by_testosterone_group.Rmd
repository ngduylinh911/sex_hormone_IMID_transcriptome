---
title: "DEGs analysis based on testosterone group in muscle dataset"
...
metadata.subset <- metadata %>%
  dplyr::rename(sex = 1, age = 2, testosterone =4) %>%
  dplyr::filter(sex == "M")
```
...
# Assign 'high' and 'low' based on age and testosterone thresholds
metadata.subset$group[metadata.subset$testosterone >= 420] <- 'high'
metadata.subset$group[metadata.subset$testosterone < 420] <- 'low'
...
```{r}
keep <- rowSums(counts(dds) >= 10) >= 18 # keep rows with at least 10 counts in at least 12 samples
dds <- dds[keep, ]
dds
```
...
write.csv(normalized_counts, file = "../result/muscle/testosterone_normalized_counts.csv",)
...
ggsave("../result/muscle/img/testosterone_volcano_plot.png", plot = volcano_plot, width = 6, height = 6, dpi = 300)
...
```{r}
write.csv(res.mod, file = "../result/muscle/testosterone_DEGs_result.csv", row.names = TRUE)
```
...
# write.csv(vst_degs_by_pvalue, file = "../result/muscle/testosterone_vst_DEGs_expression_by_pvalue.csv", row.names = TRUE)
write.csv(metadata.subset, file = "../result/muscle/testosterone_clinical_data.csv")
```
...
write.csv(all_rld_matrix, file = "../result/muscle/testosterone_vst_all_genes.csv",row.names = TRUE)
```