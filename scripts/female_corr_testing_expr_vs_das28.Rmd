---
title: "Correlation Analysis of Female Rheumatoid Arthritis Patients"
---
...
de_genes_df <- read.csv("../result/pbmc/estrogen_significant_DEGs.csv", row.names = 1)
head(de_genes_df)
...
We load the gene expression data and corresponding metadata for female rheumatoid arthritis (RA) patients.

```{r}

expression_df <- read.csv("../result/RA/female_RA_expression_data.csv", row.names = 1)
metadata_df <- read.csv("../result/RA/female_RA_metadata.csv", row.names = 1)
...
The metadata is filtered to include only female patients with rheumatoid arthritis, and with an age between 20 and 36, inclusive. Samples with missing DAS28 scores are also removed. The expression data is then filtered to match the remaining samples.

```{r}

# Filter metadata for specified criteria and remove NA values in DAS28 scores
filtered_metadata <- metadata_df %>%
  filter(sex == "Female", age >= 20, age <= 36) %>%
...
write.csv(results_df, "../result/pbmc/corr_testing_result_estrogen_DEGs_vs_DAS28.csv", row.names = FALSE)
...
if (length(significant_genes) > 0) {
  ...
  for (gene_to_plot in significant_genes) {
    ...
    ggsave(filename = paste0("../result/pbmc/correlation_testing_result/estrogen_DEGs/correlation_plot_", gene_to_plot, ".png"), plot = p, width = 7, height = 5, dpi = 300)
    # Print the plot
    print(p)
  }
} else {
